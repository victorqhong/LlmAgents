...
public static class Login
{
    ...
    public static async Task<string?> GetHubLoginToken(IAgentCommunication communication, Uri agentHubUri, ILogger logger, CancellationToken cancellationToken)
    {
        ...
        if (storedToken == null)
        {
            return await PerformOAuthFlow(communication, agentHubUri, logger, cancellationToken);
        }
        ...
        var accessToken = await RefreshHubLoginToken(agentHubUri, logger, cancellationToken);
        if (accessToken != null)
        {
            return accessToken;
        }
    ...
    public static async Task<string?> RefreshHubLoginToken(Uri agentHubUri, ILogger logger, CancellationToken cancellationToken)
    {
        var storedToken = HubAuthTokenStore.LoadToken();
        ...
        {
            logger.LogWarning("Could not connect to agent manager: {message}", e.Message);
        }
        catch (Exception e)
        {
            logger.LogWarning(e, "Could not refresh hub token");
        }
    ...
    public static async Task<string?> PerformOAuthFlow(IAgentCommunication communication, Uri agentHubUri, ILogger logger, CancellationToken cancellationToken)
    {
        try
        {
            ...
            foreach (var c in messageContents)
            {
                ...
                startAuthentication = string.Equals("y", textContent.Text, StringComparison.CurrentCultureIgnoreCase);
            }
        ...
        {
            logger.LogWarning("Could not login token: {message}", e.Message);
        }
        catch (Exception e)
        {
            logger.LogWarning(e, "Could not get login token");
        }
